...
class App:
    def __init__(self, root):
        ...
        self.cb_start = ctk.CTkComboBox(
            self.sidebar, variable=self.sv, values=NAMES, width=320, height=35
        )
        ...
        self.cb_end = ctk.CTkComboBox(
            self.sidebar, variable=self.ev, values=NAMES, width=320, height=35
        )
        ...
        self.lbl_status = ctk.CTkLabel(
            self.sidebar, textvariable=self.status, text_color="gray", wraplength=320
        )
    ...
    def _draw_base(self):
        ...
        self.ax.set_title(
            "Collision Risk Intelligent Map",
            color="#e0e0e0",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _draw_route(self, r, sn, en):
        ...
        for path, color, ls, lw, label in [
            ...
            (
                ...
                "-.",
                3.0,
                f"Fastest: {r['fastest']['time_mins']:.1f}m",
            ),
        ]:
            lats, lons = zip(*path)
            self.ax.plot(
                lons, lats, color=color, ls=ls, lw=lw, alpha=0.9, label=label, zorder=3
            )
        ...
        # Start marker
        self.ax.scatter(
            sp[1], sp[0], c="#3498db", s=140, zorder=5, edgecolors="white", lw=2
        )
        # End marker
        self.ax.scatter(
            ep[1],
        ...
        all_lats = [
            p[0]
            for p in r["shortest"]["path"] + r["safest"]["path"] + r["fastest"]["path"]
        ]
        all_lons = [
            p[1]
            for p in r["shortest"]["path"] + r["safest"]["path"] + r["fastest"]["path"]
    ...
    def _show_results(self, r):
        ...
        sh, sa, fa = r["shortest"], r["safest"], r["fastest"]
        ...
        ctk.CTkLabel(
            self.res_frame,
            text=f"• Est. Time: {fa['time_mins']:.1f} mins\n• Distance: {fa['distance_km']:.2f} km\n• Risk Score: {fa['risk_sum']:.3f}",
            justify="left",
            font=ctk.CTkFont(size=13),
        ).pack(anchor="w", padx=10)

        ctk.CTkFrame(self.res_frame, height=2, fg_color="#444466").pack(
            fill="x", pady=10
        )
        ...
        ctk.CTkLabel(
            self.res_frame,
            text=f"• Est. Time: {sa['time_mins']:.1f} mins\n• Distance: {sa['distance_km']:.2f} km\n• Risk Score: {sa['risk_sum']:.3f}",
            justify="left",
            font=ctk.CTkFont(size=13),
        ).pack(anchor="w", padx=10)
        ...
        ctk.CTkLabel(
            self.res_frame,
            text=f"• Est. Time: {sh['time_mins']:.1f} mins\n• Distance: {sh['distance_km']:.2f} km\n• Risk Score: {sh['risk_sum']:.3f}",
            justify="left",
            font=ctk.CTkFont(size=13),
        ).pack(anchor="w", padx=10)
        ...
        time_diff_safe_fast = sa["time_mins"] - fa["time_mins"]
        ctk.CTkLabel(
            self.res_frame, text=f"{rr:.1f}%", font=ctk.CTkFont(size=32, weight="bold")
        ).pack(anchor="w", pady=(0, 0))
        ctk.CTkLabel(
            self.res_frame,
            text="Overall Risk Reduction",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#2ecc71",
        ).pack(anchor="w")
        ...
        ctk.CTkLabel(
            self.res_frame,
            text=f"(+{time_diff_safe_fast:.1f} mins vs fastest)",
    ...
    def _run(self):
        ...
        if (
            not sn
            or not en
            or sn == "Type an address or select..."
            or en == "Type an address or select..."
        ):
