...
class App:
    def __init__(self, root):
        ...
        if not os.path.exists("outputs/risk_grid.csv"):
            messagebox.showerror("Error", "Run main.py first."); sys.exit(1)
        ...
        s.configure("T.TLabel", background=BG, foreground="#e94560", font=("Segoe UI", 16, "bold"))

        # Header
        ttk.Label(root, text="Toronto Safe Routing", style="T.TLabel").pack(anchor="w", padx=20, pady=(10,5))
        ...
        ttk.Combobox(ctrl, textvariable=self.sv, values=NAMES, state="readonly", width=32).pack(fill="x")
        ttk.Label(ctrl, text="End:", style="I.TLabel").pack(anchor="w", pady=(8,2))
        self.ev = tk.StringVar(value=NAMES[3])
        ttk.Combobox(ctrl, textvariable=self.ev, values=NAMES, state="readonly", width=32).pack(fill="x")
        ttk.Button(ctrl, text="FIND ROUTES", command=self._run).pack(fill="x", pady=(12,5))
        self.status = tk.StringVar(value="")
        ttk.Label(ctrl, textvariable=self.status, style="I.TLabel", wraplength=300).pack(anchor="w")
    ...
    def _draw_base(self):
        ...
        shore_x = [-79.65,-79.55,-79.50,-79.45,-79.40,-79.38,-79.35,-79.30,-79.25,-79.20,-79.15,-79.10]
        shore_y = [43.58,43.585,43.60,43.62,43.63,43.635,43.64,43.66,43.69,43.73,43.74,43.75]
        self.ax.fill_between(shore_x, shore_y, 43.55, color="#1a3a5c", alpha=0.3)
        self.ax.set_xlim(-79.65,-79.10); self.ax.set_ylim(43.58,43.86)
        ...
        for sp in self.ax.spines.values(): sp.set_color("#333355")
    ...
    def _draw_route(self, r, sn, en):
        ...
        for path, color, ls, lw, label in [
            ...
            (r["safest"]["path"], "#2ecc71", "-", 3, f'Safest: {r["safest"]["distance_km"]:.1f}km'),
        ]:
            lats, lons = zip(*path)
            self.ax.plot(lons, lats, color=color, ls=ls, lw=lw, alpha=0.9, label=label, zorder=3)

        sp, ep = r["shortest"]["path"][0], r["shortest"]["path"][-1]
        self.ax.scatter(sp[1], sp[0], c="#3498db", s=120, zorder=5, edgecolors="white", lw=2)
        self.ax.scatter(ep[1], ep[0], c="#e67e22", s=150, zorder=5, edgecolors="white", lw=2, marker="*")
        self.ax.annotate(sn, (sp[1],sp[0]), fontsize=7, fontweight="bold", color="#3498db",
                        xytext=(8,10), textcoords="offset points")
        self.ax.annotate(en, (ep[1],ep[0]), fontsize=7, fontweight="bold", color="#e67e22",
                        xytext=(8,10), textcoords="offset points")
        ...
        self.ax.legend(loc="lower right", fontsize=9, facecolor=PANEL, edgecolor="#333355", labelcolor="#e0e0e0")
        self.fig.tight_layout(); self.canvas.draw()
    ...
    def _show_results(self, r):
        ...
        ttk.Label(self.res_inner, text=f"  {sh['distance_km']:.2f} km  |  Risk: {sh['risk_sum']:.3f}",
        ...
        ttk.Label(self.res_inner, text=f"  {sa['distance_km']:.2f} km  |  Risk: {sa['risk_sum']:.3f}",
                 style="I.TLabel").pack(anchor="w")
        ttk.Separator(self.res_inner).pack(fill="x", pady=8)
        ...
        ttk.Label(self.res_inner, text=f"+{ed:.1f}% extra distance", style="O.TLabel").pack(anchor="w", pady=(8,0))

    def _run(self):
        sn, en = self.sv.get(), self.ev.get()
        if sn == en:
            messagebox.showwarning("Error", "Pick different locations."); return
        self.status.set(f"Computing...")
        self.root.update()
        try:
